import type { ContactFormData, ContactApiResponse, ContactApiError } from '@/types/contact'
...
class ContactApiClient {
  ...
  async sendContactForm(data: ContactFormData): Promise<ContactApiResponse> {
    ...
    try {
      ...
      const result = await response.json() as ContactApiResponse | ContactApiError
      ...
      throw new Error('An unexpected error occurred while sending the contact form')
    }
  }
